...
public:
	...
	__host__ __device__ HeterogeneousMediumWithVariableBoundary(const HeterogeneousMediumWithVariableBoundary& medium) :
	...
	__device__ __forceinline__ bool sampleDistance(float3& ray_o, float3& ray_d, float& dist, Rng& rng, float& sampled_distance) const {
		...
		sampled_distance = woodcockTracking(
			...
			max_density,
			density_volume,
			density_AABB.getExtent(),
			density_AABB.box_min,
			scale,
			&rng
		);
	...
	__device__ __forceinline__ bool intersect(float3& ray_o, float3& ray_d, Isect& out_result) {
		...
		while (norm3df(gradient.x, gradient.y, gradient.z) < density_threshold && new_dist>0) {
...
struct SimpleVolumeDeviceScene {
	typedef SimpleIsect SceneIsect;

	SimpleVolumeDeviceScene() {}
	Medium medium;
	Bsdf bsdf;

	__device__ __forceinline__ bool intersect(float3& ray_o, float3& ray_d, SceneIsect& out_result) {
		return medium.intersect(ray_o, ray_d, out_result);
	}

	__device__ __forceinline__ float4 Le(const float3& ray_o, const float3& ray_d, SceneIsect& out_result) {
	...
	__device__ __forceinline__ Bsdf& getBsdf(SceneIsect& isect) {
		return bsdf;
	}
};
