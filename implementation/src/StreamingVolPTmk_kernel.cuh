...
#pragma unroll
	...
	KERNEL_LAUNCH d_extend(Threads d_threads_in, Threads d_threads_out, float4* d_output, Scene scene, Rng::State* states, bool* active_threads) {
...
#pragma unroll
		...
		for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ITEM++) {
			if (thread[ITEM].active) {
				...
				d_threads_out.paths.throughputs[items_id[ITEM]] = thread[ITEM].path.throughput;
