...
__host__ __device__ inline void generateLocalBasis(const float3 &_dir, float3 &_vec1, float3 &_vec2)
{
...
__host__ __device__ inline float3 sphericalDirection(float _sinTheta, float _cosTheta, float _phi,
...
__host__ __device__ inline float PhaseHG(const float3 &_vecIn, float3 &_vecOut, float _g)
{
...
__host__ __device__ inline float PdfHG(const float3 &_vecIn, float3 &_vecOut, float _g)
{
	return PhaseHG(_vecIn, _vecOut, _g);
}

__host__ __device__ inline float3 ImportanceSampleHG(const float3 &_v, float _g, float e1, float e2)
{
	float cosTheta;
	if (fabsf(_g) > EPSILON)
	{
