...
namespace StreamingVolPTsk_kernel {
	...
	template <int BLOCK_THREADS, int ITEMS_PER_THREAD, class Scene, class Isect, Variant VARIANT>
	struct BlockStreamingVolPT {
		...
		__device__ __forceinline__ BlockStreamingVolPT(TempStorage& _temp_storage) :temp_storage(_temp_storage)
		{}
		...
		__device__ __forceinline__ void scatterThread(Thread (&thread)[ITEMS_PER_THREAD], Threads& d_threads , int (rank)[ITEMS_PER_THREAD]) {
		...
		__device__ __forceinline__ void swapThread(Thread (&thread)[ITEMS_PER_THREAD], Threads& d_threads, int rank[ITEMS_PER_THREAD]) {
...
#pragma unroll
		...
		__device__ __forceinline__ void scanAndCompact(Thread (&thread) [ITEMS_PER_THREAD], Threads& d_threads, int& n_active) {
...
#pragma unroll
		...
		__device__ __forceinline__ void compact(AABB& aabb, Thread(&thread)[ITEMS_PER_THREAD], Threads d_threads, int& n_active);


		template <>
		__device__ __forceinline__ void compact<kClassic>(AABB& aabb, Thread(&thread)[ITEMS_PER_THREAD], Threads d_threads, int& n_active)
		{
			scanAndCompact(thread, d_threads, n_active);
		}

		template <>
		__device__ __forceinline__ void compact<kSortingRays>(AABB& aabb, Thread (&thread) [ITEMS_PER_THREAD], Threads d_threads, int& n_active)
		{
			...
			BlockMortonSortT(temp_storage.sort).mortonSort(t_ray_o, aabb, rank, t_active);
...
#endif
					} while (d_paths_head_global > c_n_paths && active);//while paths are not regenerated anymore and thread is still active
				} // if active check
			}
		...
		__device__ __forceinline__ void loadThread(Thread (&thread) [ITEMS_PER_THREAD], Threads d_threads) {
			...
			BlockLoadFloat3T(temp_storage.load.tfloat3_0).Load(d_threads.paths.rays.o, t_ray_o);
			__syncthreads();

			BlockLoadFloat3T(temp_storage.load.tfloat3_1).Load(d_threads.paths.rays.d, t_ray_d);
			__syncthreads();

			BlockLoadFloat4T(temp_storage.load.tfloat4).Load(d_threads.paths.throughputs, t_throughput);
			__syncthreads();

			BlockLoadUintT(temp_storage.load.tuint).Load(d_threads.image_ids, t_image_id);
			__syncthreads();
	...
	template <int BLOCK_THREADS, int ITEMS_PER_THREAD = 1, typename Scene, Variant VARIANT = kSortingRays>
	KERNEL_LAUNCH d_render(Threads d_threads, float4* d_output, Scene scene) {

		typedef BlockStreamingVolPT<BLOCK_THREADS, ITEMS_PER_THREAD, Scene, typename Scene::SceneIsect, VARIANT> BlockStreamingVolPT_T;
