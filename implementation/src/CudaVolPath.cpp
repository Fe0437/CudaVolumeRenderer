...
template <class VolPathKernelLauncher>
CudaVolPath<VolPathKernelLauncher>::CudaVolPath(const Config& config, OutputDelegate* output_delegate):
	scene_(config.scene),
	path_tracing_config_(config.path_tracing_config),
	tiling_config_(config.tiling_config),
	cuda_config_(config.cuda_config),
	output_delegate_(output_delegate)
{
	initTileArray();

	kernel_launcher_.copyRasterToView(
		scene_.getCamera()->getRasterToView()
	);

	cudaEventCreateWithFlags(&current_buffer_ready_event_, cudaEventDisableTiming);

#ifdef DOUBLE_BUFFERING
	cudaEventCreateWithFlags(&processing_buffer_ready_event_, cudaEventDisableTiming);
#endif

	kernel_launcher_.setCudaConfig(config.cuda_config);
	kernel_launcher_.setResolution(tiling_config_.tile_dim);
	kernel_launcher_.copyPixelIndexRange(make_float2(tiling_config_.resolution.x, tiling_config_.resolution.y));
...
void CudaVolPath<VolPathKernelLauncher>::initDeviceScene(){
	...
	VolPathKernelLauncher::DeviceScene device_scene;
	auto& device_medium = device_scene.medium;

	device_medium.albedo_volume.volume_tex = createTextureWithVolume<float4>(d_albedo_, albedo_volume.getVolumeData(), albedo_volume.getCudaExtent());
	device_medium.albedo_volume.grid_resolution = medium.albedo_volume.grid_resolution;

	device_medium.density_volume.volume_tex = createTextureWithVolume<float>(d_density_,  density_volume.getVolumeData(), density_volume.getCudaExtent());
	device_medium.density_volume.grid_resolution = medium.density_volume.grid_resolution;
...
cudaTextureObject_t CudaVolPath<VolPathKernelLauncher>::createTextureWithVolume(VolumeDataPointer volume_data, VolumeType *h_volume, cudaExtent volumeSize)
{
	cudaTextureObject_t texture=0;
	cudaResourceDesc    texRes;
	memset(&texRes, 0, sizeof(cudaResourceDesc));

	if (cuda_config_.unified_memory) { //not enough global memory for the texture
		...
		if (cuda_config_.device_properties.canMapHostMemory) {
			...
			size_t bytes = volumeSize.width * volumeSize.height * volumeSize.depth * (sizeof(VolumeType));
			checkCudaErrors(cudaHostAlloc((void**)&volume_data.cuda_malloc, bytes, cudaHostAllocMapped));
			cudaMemcpy(volume_data.cuda_malloc, h_volume, bytes, cudaMemcpyHostToHost);
		...
		// create 3D array
		checkCudaErrors(cudaMalloc3DArray(&volume_data.cuda_array, &channelDesc, volumeSize));
...
void CudaVolPath<VolPathKernelLauncher>::prepareForNextIterations() {
	...
	if (tiles_.size() != 1) { // with only 1 tile the buffer is used for accumulate the result
		int tot_pixels = tiling_config_.tile_dim.x* tiling_config_.tile_dim.y;
		checkCudaErrors(cudaMemset(d_output_processed_, 0, tot_pixels * sizeof(float4)));
	}
}
...
#ifdef DOUBLE_BUFFERING
	if(tiles_.size()!=1)
		checkCudaErrors(cudaMalloc((void **)&d_output_processed_, tot_pixels * sizeof(float4)));
	else
	{
...
//render next tile at the end swap the buffer for processing (next time will use another one)
template <class VolPathKernelLauncher>
void CudaVolPath<VolPathKernelLauncher>::runIterations(){
	...
	LOG_DEBUG_IF(
		tiles_.size()!=1,
		"tile :  %d ,  %d  \n", (int)(tile_start.x / tiling_config_.tile_dim.x), (int)(tile_start.y / tiling_config_.tile_dim.y
	))
...
template <class VolPathKernelLauncher>
void CudaVolPath<VolPathKernelLauncher>::getImage(void* buffer_out) { //get last tile image
	
	assert(output_delegate_);

	uint2 tile_start = *(current_tile_-1);
	size_t offset_out = (tile_start.x + tile_start.y * tiling_config_.resolution.x) * sizeof(float4);
	
	size_t pitch_in = tiling_config_.tile_dim.x * sizeof(float4);
	size_t pitch_out = tiling_config_.resolution.x * sizeof(float4);

	size_t width = tiling_config_.tile_dim.x * sizeof(float4);
	size_t height = tiling_config_.tile_dim.y;

	output_delegate_->transfer(d_output_, pitch_in, buffer_out, offset_out, pitch_out, width, height, UtilityFunctors::Scale(current_iteration_));
...
void CudaVolPath<VolPathKernelLauncher>::releaseDeviceMemory() {
	...
	if(d_albedo_.cuda_array != 0) checkCudaErrors(cudaFreeArray(d_albedo_.cuda_array));
	if(d_density_.cuda_array != 0) checkCudaErrors(cudaFreeArray(d_density_.cuda_array));
	if (d_albedo_.cuda_malloc != 0) checkCudaErrors(cudaFree(d_albedo_.cuda_malloc));
	if (d_density_.cuda_malloc != 0) checkCudaErrors(cudaFree(d_density_.cuda_malloc));

	for (auto& txt = volume_textures_.begin(); txt != volume_textures_.end(); txt++)
...
template <class VolPathKernelLauncher>
void CudaVolPath<VolPathKernelLauncher>::render(void* buffer_out){
