...
namespace simple_StreamingVolPTsk_kernel {
	...
	template <int BLOCK_THREADS, int ELEMENTS_PER_THREAD, class BSDF, Variant VARIANT = kSortingRays>
	struct BlockStreamingVolPT {

		typedef cub::BlockLoad<float3, BLOCK_THREADS, ELEMENTS_PER_THREAD, cub::BLOCK_LOAD_WARP_TRANSPOSE> BlockLoadFloat3T;
		typedef cub::BlockLoad<float4, BLOCK_THREADS, ELEMENTS_PER_THREAD, cub::BLOCK_LOAD_WARP_TRANSPOSE> BlockLoadFloat4T;
		typedef cub::BlockLoad<uint, BLOCK_THREADS, ELEMENTS_PER_THREAD, cub::BLOCK_LOAD_WARP_TRANSPOSE> BlockLoadUintT;
		typedef cub::BlockLoad<bool, BLOCK_THREADS, ELEMENTS_PER_THREAD, cub::BLOCK_LOAD_WARP_TRANSPOSE> BlockLoadBoolT;
		typedef cub::BlockScan<int, BLOCK_THREADS, cub::BlockScanAlgorithm::BLOCK_SCAN_RAKING> BlockScanT;
		typedef BlockMortonSort<BLOCK_THREADS, ELEMENTS_PER_THREAD, int> BlockMortonSortT;
		...
		__device__ __forceinline__ void regenerate(Thread& thread, Threads d_threads, Rng& rng) {
			...
			if (threadIdx.x >= d_n_active[blockIdx.x]) {
				...
				thread.path.ray = indexToCameraRay(pixel_index, c_pixel_index_range, c_raster_to_view, c_inv_view_mat, rng);
		...
		__device__ __forceinline__ void scanAndCompact(Thread(&_thread)[ELEMENTS_PER_THREAD], Threads& d_threads) {

			auto& thread = _thread[0];
			int active_position = (int)thread.active;
			int aggregate = 0;

			BlockScanT(temp_storage.scan).ExclusiveSum(active_position, active_position, aggregate);
		...
		__device__ __forceinline__ void compact(Medium& medium, Thread(&thread)[ELEMENTS_PER_THREAD], Threads& d_threads);


		template <>
		__device__ __forceinline__ void compact<kClassic>(Medium& medium, Thread(&thread)[ELEMENTS_PER_THREAD], Threads& d_threads)
		{
			scanAndCompact(thread, d_threads);
		}

		template <>
		__device__ __forceinline__ void compact<kSortingRays>(Medium& medium, Thread(&thread)[ELEMENTS_PER_THREAD], Threads& d_threads)
		{
			
			int index[1];
			index[0] = threadIdx.x;

			int aggregate = BlockReduceT(temp_storage.reduce).Sum((int)thread[0].active);
			...
			BlockMortonSortT(temp_storage.sort).mortonSort(thread[0].path.ray.o, medium.density_AABB, index, thread[0].active);
		...
		__device__ __forceinline__ void extend(Thread(&thread)[ELEMENTS_PER_THREAD], Threads d_threads, float4* d_output, Medium medium, BSDF bsdf, Rng& rng) {
			...
			if (active) {
				...
				do {
					...
					else
					{
						...
						if (!isect.inside_volume || !medium.sampleDistance(ray_o, ray_d, isect.dist, rng, sampled_distance)) {
							...
							float4 albedo = tex3D<float4>(medium.albedo_volume.volume_tex, int(coord.x), int(coord.y), int(coord.z));
		...
		__device__ __forceinline__ void loadThread(Thread& thread, Threads d_threads) {
			...
			BlockLoadFloat3T(temp_storage.load.tfloat3).Load(d_threads.paths.rays.o, t_ray_o);
			BlockLoadFloat3T(temp_storage.load.tfloat3).Load(d_threads.paths.rays.d, t_ray_d);
	...
	__global__ void d_render(Threads d_threads, float4* d_output, Medium medium, BSDF bsdf) {
		
		typedef BlockStreamingVolPT<BLOCK_THREADS, 1, BSDF, VARIANT> BlockStreamingVolPT_T;
