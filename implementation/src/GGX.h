...
__host__ __device__ inline float fresnelDielectric(float eta, float ndotwi, float* p_ndotwt)
{
	if (eta == 1) {
		*p_ndotwt = -ndotwi;
		return 0.0;
...
__host__ __device__ inline void reflect(
	float ndotwi,
	float3 wi,
	float3 wh,
	float3* wo
...
__host__ __device__ inline void refract(float eta, float ndotwi, float ndotwt, float3 wi, float3 n, float3* wo) {
	if (ndotwt < 0)
		eta = 1 / eta;
...
/// Full Fresnel equations
__host__ __device__ inline float fresnelDielectric(float etai, float etat, float ndotwi, float ndotwt)
{
...
__host__ __device__ inline void mitsuba_GGX_sampleVNDF(float3 _wi, float2 alpha, float2 sample, float3* wo) {

	/* Step 1: stretch wi */
	float3 wi = normalize(make_float3(
		alpha.x * _wi.x,
		alpha.y * _wi.y,
		_wi.z
	));
	...
	float normalization = 
		(float)1.f / sqrtf(slope.x*slope.x + slope.y*slope.y + (float) 1.0f);
...
*/
__host__ __device__ inline void GGX_sampleVNDF(float3 wi, float2 alpha, float2 sample, float3* wo) {

	float3 alpha_extended = make_float3(alpha, 1);
	float3 streched_wi = normalize(wi*alpha_extended);

	//generate orthonormal basis
	float3 T1 = (wi.z < 0.9999) ? normalize(cross(wi, make_float3(0, 0, 1))) : make_float3(1, 0, 0);
	float3 T2 = cross(T1, wi);

	//sample point with polar coordinates (r, phi)
	float a = 1.0 / (1.0 + wi.z);
	float r = sqrtf(sample.x);
	float phi = (sample.y < a) ? sample.y / a * PI : PI + (sample.y - a) / (1.0 - a) * PI;
	float P1 = r*cosf(phi);
	float P2 = r*sin(phi) * ((sample.y < a) ? 1.0 : wi.z);

	//compute normal
	float3 N = P1*T1 + P2*T2 + sqrtf(max(0.0, 1.0 - P1*P1 - P2*P2))*streched_wi;
...
__host__ __device__ inline float projectRoughness(const float3 &v, const float2& alpha){
...
__host__ __device__ inline float GGX_G1(const float2& alpha, const float3 &v, const float3 &m) {
...
__host__ __device__ inline float GGX_G(float2 roughness, float3 wi, float3 wo, float3 wh)
{
...
__host__ __device__ inline bool GGX_sample(
	float2 roughness,
	//intIor / extIor
	float eta,	
	// Incoming direction
