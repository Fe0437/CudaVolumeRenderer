...
struct BlockSortingVolPT {
  ...
  __device__ __forceinline__ BlockSortingVolPT(TempStorage& _temp_storage)
  ...
  __device__ __forceinline__ void swapThreadAndAccessTexture(
      Scene& scene, Thread (&thread)[ITEMS_PER_THREAD], Threads& d_threads,
...
#pragma unroll
  ...
  __device__ __forceinline__ void sortAndAccessTexture(
      Scene& scene, Thread (&thread)[ITEMS_PER_THREAD], Threads d_threads,
      int& n_active) {
    ...
    BlockMortonSortT(temp_storage.cub.sort)
        .mortonSort(t_ray_o, scene.medium.density_AABB, rank, t_active);
  ...
  __device__ __forceinline__ void extend(Thread (&thread)[ITEMS_PER_THREAD],
    ...
    for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ITEM++) {
      ...
      temp_storage.texture_access[threadIdx.x * ITEMS_PER_THREAD + ITEM] =
          false;
          ...
          if (!scene.intersect(ray_o, ray_d, isect)) {
            ...
            if (medium == 0 || !medium->sampleDistance(ray_o, ray_d, isect.dist,
              ...
              if (should_regenerate) {
                ...
                temp_storage
                    .texture_access[threadIdx.x * ITEMS_PER_THREAD + ITEM] =
                    true;
              } else {
  ...
  __device__ __forceinline__ void loadThread(Thread (&thread)[ITEMS_PER_THREAD],
    ...
    BlockLoadFloat3T(temp_storage.cub.load.tfloat3_0)
        .Load(d_threads.paths.rays.o, t_ray_o);
    __syncthreads();

    BlockLoadFloat3T(temp_storage.cub.load.tfloat3_1)
        .Load(d_threads.paths.rays.d, t_ray_d);
    __syncthreads();

    BlockLoadFloat4T(temp_storage.cub.load.tfloat4)
        .Load(d_threads.paths.throughputs, t_throughput);
    __syncthreads();

    BlockLoadUintT(temp_storage.cub.load.tuint)
...
KERNEL_LAUNCH d_render(Threads d_threads, float4* d_output, Scene scene) {
  typedef BlockSortingVolPT<BLOCK_THREADS, ITEMS_PER_THREAD, Scene,
                            typename Scene::SceneIsect>
      BlockSortingVolPT_T;
