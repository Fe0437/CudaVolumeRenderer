#ifndef SORTING_VOLPT_SK_KERNEL_H_
#define SORTING_VOLPT_SK_KERNEL_H_
#pragma once
...
namespace SortingVolPTsk_kernel {
	...
	struct BlockSortingVolPT {

		typedef cub::BlockLoad<float3, BLOCK_THREADS, ITEMS_PER_THREAD, cub::BLOCK_LOAD_TRANSPOSE> BlockLoadFloat3T;
		typedef cub::BlockLoad<float4, BLOCK_THREADS, ITEMS_PER_THREAD, cub::BLOCK_LOAD_TRANSPOSE> BlockLoadFloat4T;
		typedef cub::BlockLoad<uint, BLOCK_THREADS, ITEMS_PER_THREAD, cub::BLOCK_LOAD_TRANSPOSE> BlockLoadUintT;

		typedef cub::BlockScan<int, BLOCK_THREADS, cub::BlockScanAlgorithm::BLOCK_SCAN_RAKING> BlockScanT;
		typedef BlockMortonSort<BLOCK_THREADS, ITEMS_PER_THREAD, int> BlockMortonSortT;
		...
		union CubTempStorage {
			Load load;
			BlockMortonSortT::TempStorage	 sort;
			BlockReduceT::TempStorage	 reduce;
		};

		struct TempStorage
		{
			typename CubTempStorage cub;
			bool texture_access[BLOCK_THREADS * ITEMS_PER_THREAD];
		};

		TempStorage& temp_storage;

		__device__ __forceinline__ BlockSortingVolPT(TempStorage& _temp_storage) :temp_storage(_temp_storage)
		{}

		__device__ __forceinline__ void regenerate(Thread(&thread)[ITEMS_PER_THREAD], Threads& d_threads, int& n_active, Rng &rng) {
			...
			for (int ITEM = 0; ITEM<ITEMS_PER_THREAD; ITEM++) {
				...
				if ((threadIdx.x * ITEMS_PER_THREAD) + ITEM >= n_active) {
					...
					Ray ray = indexToCameraRay(pixel_index, c_pixel_index_range, c_raster_to_view, c_inv_view_mat, rng);
		...
		__device__ __forceinline__ void swapThreadAndAccessTexture(Scene& scene, Thread(&thread)[ITEMS_PER_THREAD], Threads& d_threads, int rank[ITEMS_PER_THREAD]) {
...
#pragma unroll
			for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ITEM++) {
				...
				//check for delayed texture access
				if (temp_storage.texture_access[rank[ITEM]]) {
					float4 albedo = scene.medium.sampleAlbedo(thread[ITEM].path.ray.o);
					thread[ITEM].path.throughput = thread[ITEM].path.throughput * albedo;
				}
...
#pragma unroll
		...
		__device__ __forceinline__ void sortAndAccessTexture(Scene& scene, Thread(&thread)[ITEMS_PER_THREAD], Threads d_threads, int& n_active)
		{
			...
			int aggregate = BlockReduceT(temp_storage.cub.reduce).Sum(t_active);

			//sort all the threads in the block
			BlockMortonSortT(temp_storage.cub.sort).mortonSort(t_ray_o, scene.medium.density_AABB, rank, t_active);
			...
			swapThreadAndAccessTexture(scene, thread, d_threads, rank);
		}

		// n_paths = 0 && d_threads_in is full
		__device__ __forceinline__ void extend(Thread(&thread)[ITEMS_PER_THREAD], Threads d_threads, int& n_active, float4* d_output, Scene& scene, Rng &rng) {

			bool should_regenerate = d_paths_head_global <= c_n_paths;
			...
			for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ITEM++) {
				...
				temp_storage.texture_access[threadIdx.x*ITEMS_PER_THREAD + ITEM] = false;
				...
				if (active) {
					...
					do {
						...
						if (!scene.intersect(ray_o, ray_d, isect)) {
							atomicAdd(&d_output[image_id], throughput * scene.Le(ray_o, ray_d, isect));
							active = false;
						}
						else
						{
							...
							if (medium == 0 || !medium->sampleDistance(ray_o, ray_d, isect.dist, rng, sampled_distance)) {
								...
								if (should_regenerate) {
									//delay texture access
									temp_storage.texture_access[threadIdx.x*ITEMS_PER_THREAD + ITEM] = true;
								}
								else {
					...
					} while (!should_regenerate && active);//while paths are not regenerated anymore and thread is still active
				} // if active check
			}

			sortAndAccessTexture(scene, thread, d_threads, n_active);
		}

		__device__ __forceinline__ void offsetThreads(Threads& d_threads, int offset) {
		...
		__device__ __forceinline__ void loadThread(Thread(&thread)[ITEMS_PER_THREAD], Threads d_threads) {
			...
			BlockLoadFloat3T(temp_storage.cub.load.tfloat3_0).Load(d_threads.paths.rays.o, t_ray_o);
			__syncthreads();

			BlockLoadFloat3T(temp_storage.cub.load.tfloat3_1).Load(d_threads.paths.rays.d, t_ray_d);
			__syncthreads();

			BlockLoadFloat4T(temp_storage.cub.load.tfloat4).Load(d_threads.paths.throughputs, t_throughput);
			__syncthreads();

			BlockLoadUintT(temp_storage.cub.load.tuint).Load(d_threads.image_ids, t_image_id);
			__syncthreads();
		...
		__device__ __forceinline__ void render(Threads& d_threads, float4* d_output, Scene& scene) 
		{
	...
	KERNEL_LAUNCH d_render(Threads d_threads, float4* d_output, Scene scene) {
		...
		typedef BlockSortingVolPT<BLOCK_THREADS, ITEMS_PER_THREAD, Scene, typename Scene::SceneIsect> BlockSortingVolPT_T;
		__shared__ typename BlockSortingVolPT_T::TempStorage temp_storage;

		BlockSortingVolPT_T volpt(temp_storage);
