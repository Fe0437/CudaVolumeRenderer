...
public:
	...
	MhaSceneBuilder(std::string filename) {
		parse(filename);
	}
	...
	void parse(std::string mha_filepath) {
		...
		//loading into medium
		auto density_data = (float *)loadMhdFile<float>(mha_filepath, &medium_.max_density);
...
private:
	...
	void* loadMhdFile(std::string mha_filepath, float* max = 0)
	{
		...
		//auto normalized_spacing = std::min(spacing[0], std::min(spacing[1], spacing[2]));
		float normalized_spacing = 1.f;
		resample->SetOutputSpacing(normalized_spacing, normalized_spacing, normalized_spacing);
		...
		LOG_DEBUG("VOLUME SIZE %d %d %d \n", volume_size_.x, volume_size_.y, volume_size_.z)

		size_t element_size = sizeof(float);
		float* rawData = (float*) malloc(volume_size_.x* volume_size_.y* volume_size_.z* element_size);
		...
		for (uint z = 0; z < volume_size_.z; z++)
			for (uint y = 0; y < volume_size_.y; y++)
				for (uint x = 0; x < volume_size_.x; x++)
				{
					*iter = smoothStep(smooth_step_edges_.x, smooth_step_edges_.y, (*iter -_min) / (_max - _min) );
					iter++;
				}
