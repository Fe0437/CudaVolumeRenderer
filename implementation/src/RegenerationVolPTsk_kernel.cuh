		...
		do
		{
...
#else
				if (laneId == 0)
					head_warp = atomicAdd(&paths_head_global, 32); // all threads except lane 0

				head_warp = __shfl_sync(0xffffffff, head_warp, 0); // Synchronize all threads in warp, and get "value" from lane 0
			...
			if (active) {
				...
				if (!scene.intersect(path.ray.o, path.ray.d, isect)) {
					atomicAdd(&d_output[image_id], path.throughput * scene.Le(path.ray.o, path.ray.d, isect));
					active = false;
				}
				else
				{
					...
					if (medium == 0 || !medium->sampleDistance(path.ray.o, path.ray.d, isect.dist, rng, sampled_distance)) {
						...
						path.ray.o = path.ray.o + path.ray.d * sampled_distance - path.ray.d*EPSILON;
...
///----------------------------------------------SINGLE THREAD REGENERATION KERNEL ---------------------------------------------------------

	template <typename Scene, typename Isect = typename Scene::SceneIsect>
	KERNEL_LAUNCH d_render_single_thread_regeneration(float4* d_output, Scene scene) {
