...
// code from https://devblogs.nvidia.com/parallelforall/thinking-parallel-part-iii-tree-construction-gpu/
...
__host__ __device__ __forceinline__ unsigned int getMortonIndex(unsigned int code, double scale)
{
	unsigned int index = (unsigned int) (double(code) * scale);
	return index;
}

__host__ __device__ __forceinline__ unsigned int getMortonIndex(int i, int range_i, int j, int range_j, int k, int range_k)
{
	double scale_factor = double(((range_i) * (range_j) * (range_k))) / double(0x3FFFFFFFu);
	return getMortonIndex( morton3D((float)i / range_i, (float)j / range_j, (float)k / range_k), scale_factor);
}
