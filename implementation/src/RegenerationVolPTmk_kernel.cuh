...
#ifndef REGENERATION_VOLPT_MK_KERNEL_H_
#define REGENERATION_VOLPT_MK_KERNEL_H_
#pragma once
...
namespace RegenerationVolPTmk_kernel {

	__constant__ uint c_seed;
	__device__ uint d_paths_head_global = 0;

	template<int ITEMS_PER_THREAD = 1>
	__global__ void d_regenerate(Threads d_threads_out, Rng::State* states, bool* active_threads) {

#pragma unroll
		for (int ITEM = 0; ITEM<ITEMS_PER_THREAD; ITEM++) {

			uint item_id = ITEM + threadIdx.x*ITEMS_PER_THREAD + blockDim.x*blockIdx.x*ITEMS_PER_THREAD;

			//---------------REGENERATE--------------------
			if (active_threads[item_id]) {
				...
				Path path;
				path.ray = indexToCameraRay(pixel_index, c_pixel_index_range, c_raster_to_view, c_inv_view_mat, rng);
	...
	template <int BLOCK_THREADS, int ITEMS_PER_THREAD = 1, class Scene, class Isect = typename Scene::SceneIsect >
	KERNEL_LAUNCH d_extend(Threads d_threads, float4* d_output, Scene scene, Rng::State* states, bool* active_threads) {

		using namespace cub;
		typedef BlockLoad<float3, BLOCK_THREADS, ITEMS_PER_THREAD, BLOCK_LOAD_TRANSPOSE> BlockLoadFloat3T;
		typedef BlockLoad<float4, BLOCK_THREADS, ITEMS_PER_THREAD, BLOCK_LOAD_TRANSPOSE> BlockLoadFloat4T;
		typedef BlockLoad<uint, BLOCK_THREADS, ITEMS_PER_THREAD, BLOCK_LOAD_TRANSPOSE> BlockLoadUintT;
		typedef BlockLoad<bool, BLOCK_THREADS, ITEMS_PER_THREAD, BLOCK_LOAD_TRANSPOSE> BlockLoadBoolT;
		typedef BlockLoad<Rng::State, BLOCK_THREADS, 1, BLOCK_LOAD_TRANSPOSE> BlockLoadStateT;
		typedef BlockScan<int, BLOCK_THREADS, BlockScanAlgorithm::BLOCK_SCAN_RAKING> BlockScanT;

		Rng::State t_rnd_state[1];
		typename BlockLoadStateT::TempStorage   load_state;
		BlockLoadStateT(load_state).Load(states + blockDim.x*blockIdx.x, t_rnd_state);
...

		{
			...
			BlockLoadFloat3T(temp_storage.load.tfloat3_0).Load(d_threads_in.paths.rays.o + offset, t_ray_o);
			__syncthreads();
			BlockLoadFloat3T(temp_storage.load.tfloat3_1).Load(d_threads_in.paths.rays.d + offset, t_ray_d);
			__syncthreads();
			BlockLoadFloat4T(temp_storage.load.tfloat4).Load(d_threads_in.paths.throughputs + offset, t_throughput);
			__syncthreads();
			BlockLoadBoolT(temp_storage.load.tbool).Load(active_threads + offset, t_active);
			__syncthreads();
			BlockLoadUintT(temp_storage.load.tuint).Load(d_threads_in.image_ids + offset, t_image_id);
...
#pragma unroll
		...
		for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ITEM++) {
			...
			if (thread[ITEM].active) {
				d_threads_out.paths.rays.o[item_id] = thread[ITEM].path.ray.o;
				d_threads_out.paths.rays.d[item_id] = thread[ITEM].path.ray.d;
				d_threads_out.paths.throughputs[item_id] = thread[ITEM].path.throughput;
				d_threads_out.image_ids[item_id] = thread[ITEM].image_id;
			}

			active_threads[item_id] = thread[ITEM].active;
		}
	}
